...
namespace Pixel_Play.Scripts.OffScreenIndicator
{
    ...
    public class OffScreenIndicator : MonoBehaviour
    {
        ...
        void DrawIndicators()
        {
            ...
            {
                var screenPosition = OffScreenIndicatorCore.GetScreenPosition(mainCamera, target.transform.position);
                var isTargetVisible = OffScreenIndicatorCore.IsTargetVisible(screenPosition);
                var distanceFromCamera = target.NeedDistanceText ? target.GetDistanceFromCamera(mainCamera.transform.position) : float.MinValue;// Gets the target distance from the camera.
